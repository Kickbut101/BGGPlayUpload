...
function Show-Menu
{
     ...
     Write-Host "1: Choose username to read plays from" -ForegroundColor Cyan
     Write-Host "2: Add filter for games to upload (Name or Date)" -ForegroundColor Cyan
     Write-Host "3: (Optional)Manually point to xml file that has plays" -ForegroundColor Cyan
     Write-Host "4: Upload plays (Make it happen Cap'n)" -ForegroundColor Yellow
     Write-Host "5: View currently filtered plays" -ForegroundColor Cyan
     Write-Host ""
     Write-Host ""
     Write-Host "============================================="
     Write-Host "Current username: " -ForegroundColor Cyan -nonewline; write-host "$pointedUsername" -foregroundcolor red -nonewline; write-host " has " -foregroundcolor Cyan -nonewline; write-host "$numberOfTotalPlays" -foregroundcolor red -nonewline; write-host " plays logged." -foregroundcolor Cyan
     Write-Host "Number of plays about to upload " -ForegroundColor Cyan -nonewline; write-host "$numberOfFilteredPlays" -foregroundcolor red
     Write-Host "Pointing to xml file at " -ForegroundColor Cyan -nonewline; write-host "$xmlLocation" -ForegroundColor red
     Write-Host "============================================="
...
do
{
     ...
     switch ($input.toupper())
     {
           ...
           '2' # "2: Add filter for games to upload (Name)"
           {
                ...
                $p = Read-host -Prompt "Modify player [N]ame or [D]ate of play?"
                if ($p.toupper() -eq 'N')
                {
                ...
                {
                    cls
                    Write-Host "How far back do you want to go to filter plays?"
                    Write-Host "Enter date in format of MM-DD-YYYY  - Please include the hyphens"
                    $inputDate = Read-Host
                    $xmlOfPlays = $xmlOfPlays.where({ (get-date $_.date) -ge (get-date $inputDate)})
                }
           ...
           '4' # 4: Upload plays (Make it happen Cap'n)
           {
                ...
                For ($arrayNumber=0;$arrayNumber -lt (@($xmlOfPlays.id).count);$arrayNumber++)

                    {
                      ...
                      '5' # 5: View currently filtered plays
           {
                cls
                $xmlOfPlays | Sort-Object -Property date -Descending | Out-GridView
